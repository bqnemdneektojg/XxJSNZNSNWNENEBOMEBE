...
async def anti_flood(*args, **kwargs):
    m = args[0]
    await m.answer(' *❗* Замечен флуд, остановитесь.')
...
*ID*:</b> <code>{message.from_user.id}</code>
<b>*🔗 Линк*:</b> {message.from_user.get_mention()}''')
    await message.answer(f'''<b>*🕷️* Приветствую в Buddies Bomber!
<i>🕸️ Используйте кнопки ниже!: </i></b>''', reply_markup=start_keyboard(message.from_user.id))
...
<b>🌐 С доступом:</b> <code>{Users.get_subs_count()}</code>
...
async def bomber(callback_query: types.CallbackQuery):
    ...
    if not Users.have_sub(callback_query.from_user.id):
        await callback_query.message.answer(f'<b>*❗* Для запуска аттаки, необходимо купить доступ.</b>', reply_markup=menu_keyboard(callback_query.from_user.id))
    else:
        if Bombs.get_bombs_count_by_id(callback_query.from_user.id) == 3:
            await callback_query.message.answer(f'<b>*❗* У вас уже запущено несколько аттак, необходимо подождать их завершения.</b>', reply_markup=start_keyboard(message.from_user.id))
        else:
            await callback_query.message.answer(f'<b>*📞* Введите номер жертвы без (+)</b>', reply_markup=cancel_keyboard())
...
async def bomber_1(message: types.Message, state: FSMContext):
    if not Users.have_sub(message.from_user.id):
        await state.finish()
        await message.answer(f'<b>*❗* Для продолжения необходимо купить доступ</b>', reply_markup=start_keyboard(message.from_user.id))
    else:
        number = message.text
        ...
        if (number.startswith('380') and len(number) == 12) or (number.startswith('7') and len(number) == 11):
            if Whitelist.in_whitelist(number):
                await state.finish()
                await message.answer(f'<b>*❗* Номер находиться в белом листе, запустить аттаку невозможно</b>', reply_markup=start_keyboard(message.from_user.id))
                return

            async with state.proxy() as data:
                data['number'] = number

            await message.answer(f'<b>🕸️ Введите количество циклов:</b>', reply_markup=cancel_keyboard())
            await StartBomber.next()
        else:
            await state.finish()
            await message.answer(f'<b>*❗* Для того что бы успешно начать аттаку, нужно написать номер без + и с кодом страны</b>', reply_markup=start_keyboard(message.from_user.id))

@dp.message_handler(state=StartBomber.iterations)
async def bomber_2(message: types.Message, state: FSMContext):
    if not Users.have_sub(message.from_user.id):
        await state.finish()
        await message.answer(f'<b>*❗️* Для продолжения необходимо купить доступ</b>', reply_markup=start_keyboard(message.from_user.id))
    else:
...
*🕸️* Циклы: {iterations}</b>''')
            await message.answer(f'''<b>💣 Бомбер запущен! #{bomb_id}

📞 Номер телефона:</b> <code>{number}</code>
<b>*🐦‍⬛* Кол-во циклов:</b> <code>{iterations}</code>

<b>*❗* По окончанию аттаки вам придет сообщение.</b>''', reply_markup=stop_bomber_keyboard(bomb_id))
            ...
            await state.finish()
            await message.answer(f'<b>*❗* Количество циклов должно быть числом, и не больше 100!</b>', reply_markup=start_keyboard(message.from_user.id))
...
async def buy_access_payment(callback_query: types.CallbackQuery):
    ...
    if not Users.have_sub(callback_query.from_user.id):
        ...
        if callback_query.data.split(':')[1] == "cryptobot":
            ...
            await callback_query.message.answer(f'''<b>*🕸️* Чтобы получить доступ - оплатите счёт на {amount} RUB ({amount_final} USDT)

*🕷️* Ссылка на оплату: {url}
*❗️* После оплаты, нажмите "Проверить оплату"</b>''', reply_markup=check_payment_keyboard(invoice.invoice_id, callback_query.data.split(':')[1], days))
        else:
            bill = p2p.bill(amount=amount, lifetime=15, comment="Покупка доступа к боту")
            url = bill.pay_url

            await callback_query.message.answer(f'''<b>*🕸️* Чтобы получить доступ - оплатите счёт на {amount} RUB

*🕷️* Ссылка на оплату: {url}
*❗️* После оплаты, нажмите "Проверить оплату"</b>''', reply_markup=check_payment_keyboard(bill.bill_id, callback_query.data.split(':')[1], days))
    else:
        await callback_query.message.answer(f'<b>*💣* У тебя уже имеется доступ, воспользуйся бомбером!:</b>', reply_markup=start_keyboard(message.from_user.id))

@dp.callback_query_handler(text_startswith='buy_access_time:')
async def buy_access_time(callback_query: types.CallbackQuery):
    await callback_query.message.delete()
    await callback_query.message.answer(f'<b>🥷 Выберите метод оплаты:</b>', reply_markup=methods_pay_keyboard(callback_query.data.split(':')[1]))

@dp.callback_query_handler(text='buy_access')
async def buy_access(callback_query: types.CallbackQuery):
    await callback_query.message.delete()
    await callback_query.message.answer(f'<b>🕷️ Выберите время доступа:</b>', reply_markup=time_sub_keyboard())
...
async def stop_bomber(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.answer(f'<b>🎓 Бомбер был успешно остановлен!</b>', reply_markup=menu_keyboard(callback_query.from_user.id))
